...
#  1
with open('C:/Users/Moon/Desktop/btap/chuong3moi/baseball_2D.txt', 'r', encoding='utf-8-sig') as file:
    baseball = [list(map(int, line.replace('[[', '').replace(']]', '').replace('[', '').replace(']', '').replace(',', '').split())) for line in file if line.strip()]
